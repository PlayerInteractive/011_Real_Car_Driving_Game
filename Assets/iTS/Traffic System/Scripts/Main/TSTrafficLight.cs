...
public class TSTrafficLight : MonoBehaviour
{
    ...
    public class TSLight
    {
        ...
        /// The light timer.
        /// </summary>
        [XmlElement("lightTimer")] public float lightTimer = 0f;

        /// <summary>
        /// The type of the light.
        /// </summary>
        [XmlElement("lightType")] public TSTrafficLight.LightType lightType;

        /// <summary>
        /// The light time.
        /// </summary>
        [XmlElement("lightTime")] public float lightTime = 20f;

        /// <summary>
        /// The light texture.
        /// </summary>
        [XmlIgnore] public Texture2D lightTexture;

        /// <summary>
        /// The light mesh renderer.
        /// </summary>
        [XmlIgnore] public MeshRenderer lightMeshRenderer;

        /// <summary>
        /// The enable disable renderer.
        /// </summary>
        [XmlElement("enableDisableRenderer")] public bool enableDisableRenderer = false;

        /// <summary>
        /// The light game object.
        /// </summary>
        [XmlIgnore] public GameObject lightGameObject;
    ...
    public class TSPointReference
    {
        ...
        /// The lane.
        /// </summary>
        [XmlElement("lane")] public int lane;

        /// <summary>
        /// The connector.
        /// </summary>
        [XmlElement("connector")] public int connector;

        /// <summary>
        /// The point.
        /// </summary>
        [XmlElement("point")] public int point;
    }
    ...
    /// The lights.
    /// </summary>
    [SerializeField] [XmlArray("Lights")] [XmlArrayItem("Light")]
    public List<TSLight> lights = new List<TSLight>();

    /// <summary>
    /// The manager.
    /// </summary>
    [SerializeField] public TSMainManager manager;
    ...
    /// </summary>
    [SerializeField] public List<TSPointReference> pointsNormalLight = new List<TSPointReference>();
    ...
    void Start()
    {
        trafficLightID = TSUtils.GetUniqueID();
    ...
    private void RegisterRemoveLaneCallback()
    {
        if (manager == null) { return;}
        
        manager.OnRemovedLanes += RemovedLane;
    }
    ...
    private void RemovedLane(int deletedLane)
    {
        foreach (var t in pointsNormalLight)
        {
            if (t.lane > deletedLane)
            {
                t.lane--;
            }
        }
            ...
            if (lights[lightToPlay] != null)
            {
                if (!lights[lightToPlay].enableDisableRenderer)
                {
                    if (lights[lightToPlay].lightMeshRenderer != null &&
                        lights[lightToPlay].lightMeshRenderer.material != null)
                    {
                        lights[lightToPlay].lightMeshRenderer.material.SetTexture(
                            lights[lightToPlay].shaderTexturePropertyName, lights[lightToPlay].lightTexture);
                    }
                }
                else
                {
                ...
                switch (lights[lightToPlay].lightType)
                {
                    ...
                    case TSTrafficLight.LightType.Green:
                        for (int i = 0; i < pointsNormalLight.Count; i++)
                        {
                            ChangeReservation(pointsNormalLight[i], trafficLightID, -1);
                        }
    ...
    {
        var pointRefernce = point.connector == -1
            ? manager.lanes[point.lane].points[point.point]
            : manager.lanes[point.lane].connectors[point.connector].points[point.point];
        ...
        if (lID == -1)
        {
            ...
            pointRefernce.TryUnReservePoint(ID);
        }
        else
        {
            pointRefernce.TryReservePoint(ID, null, true);
            manager.lanes[point.lane].connectors[point.connector].connectorReservedByTrafficLight = true;
            manager.lanes[point.lane].connectors[point.connector].remainingGreenLightTime = -1;
        }
    }
    
    public void AddLight( GameObject lightGameObject, LightType lightType, float time)
    {
        if (lightGameObject == null) {return;}
        
        var greenLightGameObject = lightGameObject;
        lights.Add(new TSLight()
        {
            enableDisableRenderer = true, lightType = lightType,
            lightGameObject = greenLightGameObject,
            lightMeshRenderer = greenLightGameObject.GetComponent<MeshRenderer>(),
            lightTime = time
        });
    }
}