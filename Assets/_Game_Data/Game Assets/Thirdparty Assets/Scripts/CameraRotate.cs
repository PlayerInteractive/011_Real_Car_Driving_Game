...
public class CameraRotate : MonoBehaviour
{
 ...
 //   public float rotateOnOff = 1f;
    ...
    {
	    isDragging = true;
	    StartCoroutine(SetPos(-210f, 9.37f, 4f));
	    isrimSelect = false;
        maxDistance = 5.8f;
        minDistance = 5.8f;
    }
   ...
   public void SetBodyPaintPos()
    ...
    public float Speedtohidecanvas = 5;
    void Update()
    {
	    if (DragCheck)
	    {
		    if (CameraObject)
		    {
			    ...
			    {
				    CameraObject.fieldOfView -= 10f * Time.deltaTime*Speedtohidecanvas;
			    }
			    if (_Canvas && _Canvas.alpha > 0)
			    {
				    _Canvas.alpha -= 0.05f*Speedtohidecanvas;
				    isDragging = true;
