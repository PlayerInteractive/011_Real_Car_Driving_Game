...
//			}
	...
	void NOS()
	{
		...
		if(boostInput > 1.5f && _gasInput >= .2f && NoS > 2)
		{
			...
			if (_playerCameraOld.thisCam.fieldOfView < 100)
			{
				_playerCameraOld.thisCam.fieldOfView += 40f * Time.deltaTime;
		...
		else
		{
			...
			if (_playerCameraOld==null)
			{
				_playerCameraOld = LevelManager.instace.vehicleCamera.GetComponent<RCC_Camera>();
				NosEffect = _playerCameraOld.transform.GetChild(0).gameObject; 
			}
			else if (_playerCameraOld.thisCam.fieldOfView > 60)
			{
				_playerCameraOld.thisCam.fieldOfView -= 40f * Time.deltaTime;
		...
		UiManagerObject.instance.NosFiller.fillAmount = (NoS / 100f);
	}

	private RCC_Camera _playerCameraOld;
